...
namespace Wilson.ORMapper.Internals
{
	internal class Mappings
	{
		...
		internal Mappings(XmlDocument xmlMappings, CustomProvider customProvider) {
			...
			this.ParseMappings(xmlMappings);
		...
		private void ParseMappings(XmlDocument mappings) {
			...
			this.defaultNamespace = this.GetValue(root, "defaultNamespace", null);
			if (this.defaultNamespace != null && this.defaultNamespace.Length == 0) {
			...
			foreach (XmlNode subEntityNode in root.SelectNodes("subEntity")) {
				this.ParseSubEntity(subEntityNode);
			}
			foreach (XmlNode subFileNode in root.SelectNodes("file")) {
				this.ParseFile(subFileNode);
			}
		}

		// <file path="file|path|resource" [embedded="bool"] />
		private void ParseFile(XmlNode fileNode) {
			...
			if (!embedded) {
				...
				try {
					...
					stream = Mappings.GetResourceStream(path);
		...
		private void ParseEntity(XmlNode entityNode) {
			string type = this.GetValue(entityNode, "type");
			if (this.defaultNamespace != null && type.IndexOf('.') < 0) {
				type = this.defaultNamespace + "." + type;
			}
			...
			foreach (XmlNode attributeNode in entityNode.SelectNodes("relation")) {
				this.ParseRelation(entity, attributeNode);
			}
#if DEBUG_MAPPER
		...
		private void ParseSubEntity(XmlNode subEntityNode) {
			string type = this.GetValue(subEntityNode, "type");
			if (this.defaultNamespace != null && type.IndexOf('.') < 0) {
				type = this.defaultNamespace + "." + type;
			}
			string inherits = this.GetValue(subEntityNode, "inherits");
			if (this.defaultNamespace != null && inherits.IndexOf('.') < 0) {
				inherits = this.defaultNamespace + "." + inherits;
			}
			...
			foreach (XmlNode attributeNode in subEntityNode.SelectNodes("relation")) {
				this.ParseRelation(subEntity, attributeNode);
			}
#if DEBUG_MAPPER
		...
		private void ParseRelation(EntityMap entity, XmlNode attributeNode) {
			...
			string member = this.GetValue(attributeNode, "member");
			string field = this.GetValue(attributeNode, "field");
			string type = this.GetValue(attributeNode, "type");
			if (this.defaultNamespace != null && type.IndexOf('.') < 0)
			{
				type = this.defaultNamespace + "." + type;
			}
