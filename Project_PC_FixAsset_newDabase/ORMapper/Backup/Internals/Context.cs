	...
	internal class Context
	{
		...
		public Instance this[object entityObject] {
			get {
				...
				Instance instance = (Instance)this.instances[entityKey.ToString()];
				if (instance == null) {
					throw new ORMapperException("ObjectSpace: Entity Object not being Tracked - " + entityKey.ToString());
				}
		...
		internal bool IsTracked(object entityObject) {
			EntityKey entityKey = new EntityKey(this, entityObject, true);
			return this.instances.Contains(entityKey.ToString());
		}
		...
		public object GetObject(Type objectType) {
			...
			if (this.Mappings[objectType.ToString()].KeyType != KeyType.User
					&& this.Mappings[objectType.ToString()].KeyType != KeyType.Composite) {
				if (this.mappings[objectType.ToString()].AutoTrack) {
		...
		public object GetObject(Type objectType, object objectKey, bool firstLevel) {
			...
			if (this.instances.Contains(entityKey.ToString())) {
				if (this.mappings[objectType.ToString()].AutoTrack) {
					Instance instance = (Instance) this.instances[entityKey.ToString()];
			...
			if (entityObject == null) {
				Commands commands = this.Mappings.Commands(objectType.ToString());
		...
		public int GetObjectCount(ObjectQuery objectQuery) {
			Commands commands = this.Mappings.Commands(objectQuery.ObjectType.ToString());
		...
		public void StartTracking(object entityObject, InitialState initialState) {
			...
			EntityKey entityKey = new EntityKey(this, instance.EntityObject, initialState != InitialState.Inserted);
			this.StartTracking(entityKey, instance);
		}

		internal void StartTracking(Instance instance) {
			EntityKey entityKey = new EntityKey(instance.EntityObject.GetType(), instance.Key);
			this.StartTracking(entityKey, instance);
		}

		internal void StartTracking(EntityKey entityKey, Instance instance) {
			this.instances[entityKey.ToString()] = instance;
		}

		public void EndTracking(object entityObject) {
			EntityKey entityKey = new EntityKey(this, entityObject, true);
			if (this.instances.Contains(entityKey.ToString())) {
				this.instances.Remove(entityKey.ToString());
			}
		}
		...
		public object Resync(object entityObject) {
			EntityMap entity = this.mappings[entityObject.GetType().ToString()];
			for (int index = 0; index < entity.RelationCount; index++) {
				...
				if (member is ILoadOnDemand) {
					if ((member as ILoadOnDemand).IsLoaded) {
						(member as ILoadOnDemand).Resync();
					}
				}
			}
